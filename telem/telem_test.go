	...
	Describe("TimeRange", func() {
		...
		Describe("Bound", func() {
			It("Should bound the time range to the provided constraints", func() {
				tr := telem.TimeRange{
					Start: telem.TimeStamp(time.Second),
					End:   telem.TimeStamp(time.Second * 4),
				}
				bound := telem.TimeRange{
					Start: telem.TimeStamp(2 * time.Second),
					End:   telem.TimeStamp(time.Second * 3),
				}
	...
	Describe("DataRate", func() {
		...
		Describe("ByteSpan", func() {
			It("Should return the span of the provided number of bytes", func() {
				Expect(telem.DataRate(10).ByteSpan(16, telem.Float64)).To(Equal(200 * telem.Millisecond))
			})
		})
	})
})
